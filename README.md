...
Note the `defaultAddr` will be reused in other tests. 

This test does not save the intermediary encoded data to check if it is encoded correctly, because it must be if it decodes back to the original.
...
Here we can also comment on the way that the client and server were written, with the goal of precisely making it easy to write tests like this.

You see that firstly, we must create and start up the server. The client has been written to assume there is a running server to connect to, or it immediately stops. The logic of retrying connections is outside of the scope of a client library, so we must start the server first.

Next, the client. We have the initialiser, which establishes the channels and maps in the client structure, and then when it is started up, we are provided with the encode, decode and stop functions, which allow us to send requests and shut down the client.

You will recall that we wrote the `enc` and `dec` functions to return channels - this means that we can call the function and then wait on the channel in one neat statement:
...
Since we have already proven that the codec works in the tests we wrote for it, we can replicate this test, and change the calling mechanism to run over gRPC instead.
