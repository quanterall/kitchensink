...
func makeCodec(
	...
	cdc.Encoder = func(input []byte) (output string, err error) {
		...
		if len(input) < 1 {
			...
			err = protos.GetError(protos.Error_ZERO_LENGTH)
			return
		}
	...
	cdc.Check = func(input []byte) (err error) {
		...
		case input == nil:
			...
			err = protos.GetError(protos.Error_NIL_SLICE)
			return
		}
		...
		if len(input) < checkLen+1 {
			...
			// functions).
			err = protos.GetError(protos.Error_CHECK_TOO_SHORT)
		...
		if !valid {
			...
			err = protos.GetError(protos.Error_CHECK_FAILED)
		}
	...
	cdc.Decoder = func(input string) (output []byte, err error) {
		...
		if !strings.HasPrefix(input, cdc.HRP) {
			...
			err = errors.New(protos.Error_name[int32(protos.Error_INCORRECT_HUMAN_READABLE_PART)])
